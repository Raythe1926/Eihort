...
function NiceLookingTextSize( uglyTextSize )
	-- Find a text size near the given text size that
	-- does not require fractional scaling of the font
	
	local px = ScreenHeight * uglyTextSize;
	px = 15 * math.max( 1, math.floor( px / 15 + 0.3 ) );
	return px / ScreenHeight;
end

function rectUnion( rect1, ... )
	local x, y, w, h = unpack( rect1 );
	for _, el in ipairs( { ... } ) do
		local x1, y1, w1, h1 = unpack( el );
		if x1 < x then x = x1; end
		if y1 < y then y = y1; end
		if x1+w1 > x+w then w = x1-x+w1; end
		if y1+h1 > y+h then h = y1-y+h1; end
	end
	return { x, y, w, h };
end

function rectIntersect( rect1, ... )
	local x, y, w, h = unpack( rect1 );
	for _, el in ipairs( { ... } ) do
		local x1, y1, w1, h1 = unpack( el );
		if x1 > x then x = x1; end
	...
	if w < 0 then x = x + .5 * w; w = 0; end
	if h < 0 then y = y + .5 * h; h = 0; end
	return { x, y, w, h };
end
...
local composite_meta = {
	...
	-- Shrinks a composite to fit its children
	shrinkToFit = function( this, margin )
		margin = margin or 0
		local rect = { 1, 1, 0, 0 };
		for _, el in ipairs( this ) do
			if not el.invisible then
				rect = rectUnion( rect, { el.rect:getRect() } );
			end
		end
		rect = rectIntersect( { this.rect:getRect() }, rect );
		this.rect:setRect( unpack( rect ) );
	end;
...
function ui.newRoot()
	-- The root is basically a screen-covering composite which is additionally
	-- responsible for drawing the tooltip
	...
	function root.mouseMove( this, x, y )
		-- Move the mouse inside the root
		...
		if tooltip then
			...
			local vScale = NiceLookingTextSize( ui.TooltipTheme.textHt );
	...
	function root.render( this, ctx )
		...
		-- Render the tooltip
		if tooltip then
			local textScale = NiceLookingTextSize( ui.TooltipTheme.textHt );
...
local label_meta = {
	...
	changedAspect = function( this )
		this.fontHeight = NiceLookingTextSize( this.fontHeightOrig );
...
function ui.newLabel( text, fontHeight, border, x, y, w, h )
	...
	c.fontHeightOrig = fontHeight or 0.01;
	c.fontHeight = NiceLookingTextSize( c.fontHeightOrig );
...
local button_meta = {
	...
	-- Rendering is the same as labels
...
local toggle_meta = {
	...
	-- Change the state of the toggle from code
